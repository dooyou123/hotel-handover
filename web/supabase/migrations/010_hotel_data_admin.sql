-- 관리자용 호텔 데이터 초기화 / 샘플 시드 + 긴급 칸 → 진행중 통합

-- 기존 긴급 칸 카드를 진행중으로 이전 (우선순위는 유지)
update public.cards set column_id = 'progress' where column_id = 'urgent';
update public.card_templates set column_id = 'progress' where column_id = 'urgent';

-- ---------------------------------------------------------------------------
-- reset_hotel_data: 운영 데이터 전체 삭제 (호텔·프로필 유지)
-- ---------------------------------------------------------------------------
...
grant execute on function public.reset_hotel_data(uuid) to authenticated;

-- ---------------------------------------------------------------------------
-- seed_hotel_sample_data: 데모·교육용 샘플 데이터
-- ---------------------------------------------------------------------------
...
grant execute on function public.seed_hotel_sample_data(uuid) to authenticated;
