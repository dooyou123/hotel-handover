...
import { SESSION_STORAGE_KEY, SHIFTS } from '@/lib/constants';
...
export function SessionBar({ email }: SessionBarProps) {
  ...
  const [session, setSession] = useState<SessionState>({ shift: '', name: '' });
  ...
  const ready = Boolean(session.shift && session.name);
    ...
    <section className="session-bar">
      <div className="session-bar__info">
        ...
        <p className="session-bar__hint">
          교대·이름을 선택하면 추가·확인·체크리스트·어메니티 기록에 자동으로 남습니다.
        </p>
      </div>
      <div className="session-bar__controls">
        ...
        <div className={`session-bar__status${ready ? ' is-ready' : ''}`}>
          {ready ? `근무 중: ${session.shift} · ${session.name}` : '교대와 이름을 선택해 주세요'}
        </div>
