...
-- seed_hotel_sample_data: 리셋 후 샘플에도 커피 원두가 들어가게 한다.
...
begin
  ...
  insert into public.amenities (hotel_id, name, box_size, unit_size, sort_order, unit) values
    (p_hotel_id, '덴탈키트', 250, 25, 1, '개'),
    (p_hotel_id, '면도기', 500, 25, 2, '개'),
    (p_hotel_id, '빗', 500, 25, 3, '개'),
    (p_hotel_id, '입욕제(라벤더)', 500, 50, 4, '개'),
    (p_hotel_id, '입욕제(자스민)', 500, 50, 5, '개'),
    (p_hotel_id, '설탕', 1000, 100, 6, '개'),
    (p_hotel_id, '샤워캡', 1000, 100, 7, '개'),
    (p_hotel_id, '헤어밴드', 2000, 100, 8, '개'),
    (p_hotel_id, '티(잉글리시)', 1200, 100, 9, '개'),
    (p_hotel_id, '티(얼그레이)', 1200, 100, 10, '개'),
    (p_hotel_id, '티(카모마일)', 1200, 100, 11, '개'),
    (p_hotel_id, '커피스틱', 1000, 1000, 12, '개'),
    (p_hotel_id, '커피 원두', 5, 1, 13, '봉'),
    (p_hotel_id, '마스크팩', 400, 80, 14, '개'),
    (p_hotel_id, '종이컵(대)', 1200, 100, 15, '개'),
    (p_hotel_id, '종이컵(소)', 1200, 100, 16, '개')
  on conflict (hotel_id, name) do nothing;

  insert into public.amenity_inventory (hotel_id, amenity_id, quantity)
  select p_hotel_id, a.id, case when a.unit = '봉' then 10 else 500 end
  from public.amenities a
  where a.hotel_id = p_hotel_id
  on conflict (hotel_id, amenity_id) do update
